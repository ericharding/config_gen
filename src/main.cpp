...
T parseJsonValue(const json& value, const T& defaultValue) {
    if (value.is_string()) {
        ...
        if (strValue.length() > 3 && strValue[0] == '$' && strValue[1] == '{' && strValue.back() == '}') {
...
std::map<std::string, std::string> parseStringMap(const json& j, const std::string& key) {
    std::map<std::string, std::string> resultMap;

    if (!j.contains(key) || !j[key].is_object()) {
        std::cout << "Warning: JSON does not contain an object at key '" << key << "'. Returning empty map." << std::endl;
        return resultMap;
    }

    for (const auto& [mapKey, mapValue] : j[key].items()) {
        resultMap[mapKey] = parseJsonValue<std::string>(mapValue, "DEFAULT_VALUE");
    }
...
int main() {
    // Example JSON string with nested string-to-string map and environment variable references
    ...
    try {
        ...
        std::map<std::string, std::string> resultMap = parseStringMap(j, "stringMap");
        ...
        std::cout << "Name: " << parseJsonValue<std::string>(j["name"], "Unknown") << std::endl;
        std::cout << "Missing Key: " << parseJsonValue<std::string>(j["missingKey"], "Not Found") << std::endl;
    }
